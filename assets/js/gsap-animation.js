...
// 			var pbmit_var = jQuery('.heading, .heading-subheading');
// 			if (!pbmit_var.length) {
// 				return;
// 			}
// 			const quotes = document.querySelectorAll(".heading-subheading .title, .heading .title");
...
// 				var getclass = quote.closest('.heading-subheading, .heading').className;
...
function pbmit_extend_section() {
	const pbmit_elm = gsap.utils.toArray('.extend-animation');
...
function pbmit_sticky_special() {
	...
	if (jQuery('.sticky-col2-special').hasClass('.elementor-element-edit-mode')) {
		return;
	}
	ScrollTrigger.matchMedia({
		"(min-width: 1024px)": function () {
			let pbmit_sticky_2 = jQuery(".sticky-col2-special");
			let section = jQuery('.sticky-special');
...
function pbmit_sticky() {
	...
	ScrollTrigger.matchMedia({
		"(min-width: 1200px)": function () {
			let pbmit_sticky_container = jQuery(".sticky-col");
			let section = pbmit_sticky_container.closest('section');
			if (!section[0]) {
				section = pbmit_sticky_container.closest('.sticky-section');
			}
			let tl = gsap.timeline({
				scrollTrigger: {
...
function pbmit_img_animation() {
	const boxes = gsap.utils.toArray('.animation-style1,.animation-style2,.animation-style3,.animation-style4,.animation-style5,.animation-style6,.animation-style7');
	boxes.forEach(img => {
		gsap.to(img, {
			scrollTrigger: {
				trigger: img,
...
function pbmit_staticbox_hover() {
	var pbmit_var = jQuery('.element-static-box-style-1, .element-service-style-5, .element-service-style-7');
	if (!pbmit_var.length) {
		return;
	}
	pbmit_var.each(function () {
		var pbmit_Class = '.element-posts-wrapper > .ele-static-box, .swiper-hover-slide-nav .hover-inner li, article.service-style-7';
		jQuery(this)
...
var pbmit_hover_slide = function () {
	...
	if (typeof Swiper !== 'undefined') {
		...
		var pbmit_hover1 = new Swiper(".hover-image", {
			grabCursor: true,
		...
		jQuery('.main-hover-slider li').hover(function (e) {
			e.preventDefault();
...
function pbmit_tween_effect() {
	...
	const pbmit_tween = gsap.utils.toArray('.tween-effect');
